...
interface FilePreviewProps {
  ...
  isReceipt?: boolean // Add this prop
}
...
export function FilePreview({
  ...
  showDelete = true,
  className = "",
  isReceipt = false, // Default to false
}: FilePreviewProps) {
  ...
  return (
    <Card className={`relative overflow-hidden ${className}`}>
      {isImage ? (
        <div
          className={`relative ${isReceipt ? "cursor-zoom-in" : ""} ${zoomed ? "aspect-auto" : "aspect-video"}`}
          onClick={() => isReceipt && setZoomed(!zoomed)}
        >
          <img
            src={url || "/placeholder.svg"}
            alt="File preview"
            className={`w-full h-full ${zoomed ? "object-contain max-h-[80vh]" : "object-cover"}`}
            onError={() => {
