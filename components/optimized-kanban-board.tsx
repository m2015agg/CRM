        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {statusColumns.map((status, index) => (
              <div key={status} className="space-y-2">
                <div className={`flex justify-between items-center p-2 rounded-t-md ${getColumnHeaderColor(status)}`}>
                  <h3 className="font-medium">{statusDisplayNames[status] || status}</h3>
                ...
                <Droppable droppableId={index.toString()}>
                  {(provided) => (
                    <div
                      ...
                      className={`${getColumnBgColor(status)} rounded-b-md p-2 min-h-[200px]`}
                    >
