...
export async function uploadFileToBlobAction(formData: FormData) {
  try {
    ...
    const isReceipt = folder.includes("receipt") || folder === "receipts"
    ...
    // For receipts, use a more specific naming pattern
    let filename
    if (isReceipt) {
      const dateStr = new Date().toISOString().split("T")[0].replace(/-/g, "")
      const expenseType = formData.get("expenseType") || "expense"
      filename = `${folder ? folder + "/" : ""}receipt-${expenseType}-${dateStr}-${randomString}.${extension}`
    } else {
      filename = `${folder ? folder + "/" : ""}${timestamp}-${randomString}.${extension}`
    }

    console.log(`Uploading file ${originalName} to Vercel Blob as ${filename}`)

    // For receipts, consider compressing images before upload
    const fileToUpload = file
    if (isReceipt && file.type.startsWith("image/") && file.size > 1024 * 1024) {
      // This would be where you'd add image compression logic
      // For now, we'll just log that we would compress it
      console.log("Would compress receipt image before upload")
    }
